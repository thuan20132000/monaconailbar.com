...
export default function ReviewsSection({ google, salonName }: Props) {
  const { rating, reviewCount, reviews, photos, mapsUri } = google
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {photos.length > 0 && (
          <div className="mt-12 md:mt-16">
            <p className="text-[0.65rem] font-bold tracking-[0.22em] uppercase text-mauve mb-5">
              From Google Photos
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 md:gap-3">
              {photos.slice(0, 6).map((photo, i) => (
                <Link
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="relative aspect-square overflow-hidden bg-cream-200 group"
                >
                  <Image
                    src={photo.url}
                    alt={`${salonName} — Google place photo ${i + 1}`}
                    fill
